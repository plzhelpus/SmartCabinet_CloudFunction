...
exports.addMemberToGroup = functions.https.onCall((data, context) => {
  return hasAuthority("groups/" + data.groupID, context.auth.uid)
    .then(() => {
  ...
  return db
    ...
    .then(() => {
      return deleteCollection(
        "users/" + userRecord.uid + "/participated_group/",
        100
      );
    });
});
  ...
  .onDelete((snap, context) => {
    return Promise.all([
      db
        ...
        .delete(),
      deleteCollection_nested(
        "groups/" + context.params.groupID + "/admin_ref/",
        100
      ),
      deleteCollection_nested(
        "groups/" + context.params.groupID + "/member_ref/",
        100
      ),
      deleteCollection(
        "groups/" + context.params.groupID + "/cabinet_ref/",
        100
      )
    ]).then(function() {
